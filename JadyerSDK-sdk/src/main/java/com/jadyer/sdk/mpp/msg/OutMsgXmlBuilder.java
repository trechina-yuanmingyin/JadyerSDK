...
import com.jadyer.sdk.mpp.msg.out.News;
...
public class OutMsgXmlBuilder {
	private OutMsgXmlBuilder(){}
	
	public static void write(OutMsg outMsg, HttpServletResponse response){
		try{
	...
	public static String build(OutMsg outMsg){
		...
		if("text".equals(outMsg.getMsgType())){
			return buildOutTextMsg((OutTextMsg)outMsg);
		}
		if("image".equals(outMsg.getMsgType())){
			return buildOutImageMsg((OutImageMsg)outMsg);
		}
		if("news".equals(outMsg.getMsgType())){
			return buildOutNewsMsg((OutNewsMsg)outMsg);
		}
		if("transfer_customer_service".equals(outMsg.getMsgType())){
			return buildOutCustomServiceMsg((OutCustomServiceMsg)outMsg);
		}
	 ...
	 * @see -----------------------------------------------------------------------------------------------------------
	 */
	private static String buildOutTextMsg(OutTextMsg outTexgMsg){
	 ...
	 * @see -----------------------------------------------------------------------------------------------------------
	 */
	private static String buildOutImageMsg(OutImageMsg outImageMsg){
		StringBuilder sb = new StringBuilder();
		sb.append("<xml>")
	 ...
	 * @see -----------------------------------------------------------------------------------------------------------
	 */
	private static String buildOutNewsMsg(OutNewsMsg outNewsMsg){
		StringBuilder sb = new StringBuilder();
		sb.append("<xml>")
		...
		if(!outNewsMsg.getArticles().isEmpty()){
			for(News news : outNewsMsg.getArticles()){
				sb.append("<item>")
	 ...
	 * @see -----------------------------------------------------------------------------------------------------------
	 */
	private static String buildOutCustomServiceMsg(OutCustomServiceMsg outCustomServiceMsg){
		StringBuilder sb = new StringBuilder();
		sb.append("<xml>")
